...
export default function Dashboard() {
  ...
  const [selectedMonth, setSelectedMonth] = useState(new Date()); // Added missing state!
  ...
  const fetchData = useCallback(async () => {
    ...
    if (!auth.currentUser) {
      ...
      try {
        ...
        const processedData = processTransactions(transactions, selectedMonth);
  ...
  const handleAddRandomTransaction = async () => {
    ...
    const newTx = {
      ...
      amount: randomAmount,
      date: Timestamp.fromDate(new Date()), // Strictly add to the current real-world month
    };
